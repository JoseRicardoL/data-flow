...
class GTFSProcessorManager:
    def __init__(
        ...
        status_file="batch_processing/status.json",
        lambda_name=None,
        macro_job_name=None,
        macro_stops_job_name=None,
        region="eu-west-1",
    ):
        """
        ...
        try:
            with open(self.status_file, "r") as f:
                ...
                existing_combinations = {
                    self._get_combo_key(c): c
                    for c in self.status.get("combinations", [])
                }
                ...
                logger.info(
                    f"Estado de procesamiento cargado: {self.status['total']} total, "
    ...
    def run_preprocessor(self, combination):
        ...
        try:
            ...
            logger.info(
                f"Invocando preprocesador para E={combination['P_EMPRESA']}, "
            ...
            if response_payload.get("status") == "success":
                logger.info(
                    f"Preprocesamiento exitoso: execution_id={response_payload.get('execution_id')}"
                )
                ...
                combination["error"] = response_payload.get(
    ...
    def process_combination(self, combination):
        ...
        logger.info(
            f"Iniciando procesamiento para E={combination['P_EMPRESA']}, "
            f"C={combination['P_CONTR']}, V={combination['P_VERSION']}"
        )
        ...
        if not preprocessor_result:
            ...
            logger.error(
                f"Falló el preprocesamiento para E={combination['P_EMPRESA']}, "
                f"C={combination['P_CONTR']}, V={combination['P_VERSION']}"
            )
        ...
        logger.info(
            f"Iniciando job de macro para E={combination['P_EMPRESA']}, "
            f"C={combination['P_CONTR']}, V={combination['P_VERSION']}"
        )
        ...
        common_args["--JOB_NAME"] = (
            self.macro_stops_job_name
        )  # Cambiar para el job de macro_stops

        # Iniciar MacroStopsGenerator
        logger.info(
            f"Iniciando job de macro_stops para E={combination['P_EMPRESA']}, "
            f"C={combination['P_CONTR']}, V={combination['P_VERSION']}"
        )
        ...
        max_time = datetime.now() + timedelta(
            hours=4
        )  # Limite de 4 horas para evitar bloqueos
        check_interval = 30  # segundos entre verificaciones

        while (
            not (macro_completed and macro_stops_completed)
            and datetime.now() < max_time
        ):
            ...
            if not macro_stops_completed and macro_stops_job_id:
                macro_stops_status = self.check_job_status(
                    self.macro_stops_job_name, macro_stops_job_id
                )
                if macro_stops_status in ["SUCCEEDED", "FAILED", "TIMEOUT", "STOPPED"]:
                    ...
                    logger.info(
                        f"Job de macro_stops finalizado con estado: {macro_stops_status}"
                    )
        ...
        if macro_success and macro_stops_success:
            ...
            result = True
            logger.info(
                f"Procesamiento completo exitoso para E={combination['P_EMPRESA']}, "
                f"C={combination['P_CONTR']}, V={combination['P_VERSION']}"
            )
        else:
            combination["status"] = "failed"
            combination["error"] = (
                f"Macro: {combination.get('macro_status', 'UNKNOWN')}, "
                f"MacroStops: {combination.get('macro_stops_status', 'UNKNOWN')}"
            )
            result = False
            logger.error(
                f"Procesamiento fallido para E={combination['P_EMPRESA']}, "
    ...
    def process_all(self, batch_size=5, max_retries=0):
        ...
        pending_combinations = [
            c for c in self.status["combinations"] if c["status"] == "pending"
        ]
        ...
        logger.info(
            f"Iniciando procesamiento de {total_pending} combinaciones pendientes"
        )

        for i in range(0, total_pending, batch_size):
            batch = pending_combinations[i : i + batch_size]
            logger.info(
                f"Procesando lote {i//batch_size + 1} ({len(batch)} combinaciones)"
            )
        ...
        if max_retries > 0:
            logger.info(
                f"Configurados {max_retries} reintentos para combinaciones fallidas"
            )
            for retry in range(max_retries):
                failed_combinations = [
                    c for c in self.status["combinations"] if c["status"] == "failed"
                ]
                ...
                logger.info(
                    f"Reintento {retry+1}/{max_retries}: {len(failed_combinations)} combinaciones"
                )
        ...
        logger.info(
            f"Resumen: {self.status['total']} total, "
    ...
    def retry_failed(self):
        ...
        self.save_status()
        logger.info(
            f"{len(failed_combinations)} combinaciones restablecidas a pendientes"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Gestor de procesamiento de datos GTFS"
    )
    parser.add_argument("bucket", help="Nombre del bucket S3")
    parser.add_argument("--region", default="eu-west-1", help="Región de AWS")
    parser.add_argument(
        "--batch-size", type=int, default=5, help="Tamaño del lote para procesamiento"
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=0,
        help="Máximo número de reintentos para combinaciones fallidas",
    )
    parser.add_argument(
        "--retry-failed", action="store_true", help="Reintentar combinaciones fallidas"
    )
    parser.add_argument(
        "--lambda-name", help="Nombre de la función Lambda del preprocesador"
    )
    parser.add_argument("--macro-job", help="Nombre del job Glue para macro")
    parser.add_argument(
        "--macro-stops-job", help="Nombre del job Glue para macro_stops"
    )
    parser.add_argument(
        "--combinations-file",
        default="batch_processing/combinations.json",
        help="Archivo con combinaciones",
    )
    parser.add_argument(
        "--status-file",
        default="batch_processing/status.json",

...
def register_combinations(bucket_name, state_table_name, stateMachineArn, combinations_file="batch_processing/combinations.json", region="eu-west-1", start_processing=False, max_to_start=1):
    """
    ...
    for combo in combinations:
        try:
            ...
            combo_id = combo.get("id") or f"{combo['P_EMPRESA']}_{combo['P_CONTR']}_{combo['P_VERSION']}"
            ...
            try:
                ...
                if "Item" in response:
                    logger.info(f"Combinación ya registrada: {combo_id}, estado: {response['Item'].get('status', 'desconocido')}")
                    skipped += 1
                    continue
...
def main():
    parser = argparse.ArgumentParser(description="Registrar combinaciones GTFS y gestionar su procesamiento")
    parser.add_argument("operation", choices=["register", "start", "summary", "reset"], help="Operación a realizar")
    parser.add_argument("--bucket", help="Nombre del bucket S3")
    parser.add_argument("--state-table", required=True, help="Nombre de la tabla DynamoDB de estado")
    parser.add_argument("--state-machine-arn", help="ARN de la máquina de estados")
    parser.add_argument("--combinations-file", default="batch_processing/combinations.json", help="Archivo JSON con las combinaciones")
    parser.add_argument("--region", default="eu-west-1", help="Región AWS")
    parser.add_argument("--max-start", type=int, default=1, help="Número máximo de ejecuciones a iniciar")
    ...
    if args.operation == "register":
        ...
        register_combinations(args.bucket, args.state_table, args.state_machine_arn, args.combinations_file, args.region, False, 0)
        
    elif args.operation == "start":
        if not args.bucket or not args.state_machine_arn:
            parser.error("--bucket y --state-machine-arn son requeridos para la operación start")
            
        register_combinations(args.bucket, args.state_table, args.state_machine_arn, args.combinations_file, args.region, True, args.max_start)
